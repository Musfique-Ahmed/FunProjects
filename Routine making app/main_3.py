...
class RoutineManager:
    ...
    def display_sections(self, course_code):
        # Validate if course_code exists in the dataset
        if course_code not in self.data['Course Code'].values:
            print("Invalid Course Code. Please check your input and try again.")
            return

        # Display all sections of a valid course
        sections = self.data[self.data['Course Code'] == course_code]
        if sections.empty:
            print("No section found. Please check if the input was correct.")
        else:
